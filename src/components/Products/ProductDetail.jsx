...
export default function ProductDetail() {
    ...
    // Ако продуктът още не е зареден
    if (!product) return <p>Loading...</p>;

    // Безопасно форматиране на цена
    ...
    const handleEditReview = (reviewId) => {
        ...
        fetch(`http://localhost:8000/reviews.php?action=edit&productId=${product._id?.$oid || product._id}&reviewId=${reviewId}`, {
            method: "PUT",
            headers: {
                "Authorization": `Bearer ${token}`,
                "Content-Type": "application/json"
    ...
    const handleDeleteReview = (reviewId) => {
        fetch(`http://localhost:8000/reviews.php?action=delete&productId=${product._id?.$oid || product._id}&reviewId=${reviewId}`, {
            method: "DELETE",
            headers: {
                "Authorization": `Bearer ${token}`
            }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-[1400px] mx-auto my-12 p-8 bg-white rounded-xl shadow-sm">
            ...
            <div className="flex flex-col justify-center mb-24">
                ...
                <p className="text-orange-600 font-semibold text-center mb-6">
                    Price: {formattedPrice} {t("type_currency")}
                </p>

                {/* Shipping info */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <InfoBox icon="🚚" title={t("free_shipping_title")} text={t("free_shipping_p")} />
                    <InfoBox icon="🚀" title={t("express_shipping_title")} text={t("express_shipping_p")} />
                </div>
                ...
                {product.category === "cards" && (
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            <InfoBox icon="📋" title={t("cards_info_title1")} text={t("cards_info_p1")} />
                            <InfoBox icon="✉️" title={t("cards_info_title2")} text={t("cards_info_p2")} />
                        </div>
                        <DiscountBox text={`${t("cards_discount")}: 2 ${t("type_currency")}`} />
                    </>
                )}

                {/* InfoBox секция за magnets и puzzles */}
                ...
                {/* DiscountBox само за magnets */}
                {product.category === "magnets" && (
                    <DiscountBox text={`${t("magnet_discount")}: 1.50 ${t("type_currency")}`} />
